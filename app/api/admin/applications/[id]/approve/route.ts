...
// POST /api/admin/applications/[id]/approve -> set status to 'reviewed'
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const r = await col.updateOne({ _id }, { $set: { status: 'reviewed', updatedAt: new Date(), notes: note } })
    if (!r.matchedCount) return NextResponse.json({ ok: false, error: 'not found' }, { status: 404 })

    // Audit and realtime
    await logAudit({ actorId: auth.id, actorRole: auth.role, action: 'application_approve', targetType: 'application', targetId: String(_id), meta: { from: existing.status, to: 'reviewed', note } })
    broker.publish({ type: 'application.status_changed', payload: { id: String(_id), from: existing.status, to: 'reviewed' } })
    ...
    try {
      ...
      await (notiCol as any).insertOne({
        ...
        message: 'Your application has been approved for review by admin.',
